...
try {
    ...
    const response = await fetch('/api/create_super_admin', {
        method: "POST",
        headers: {
...
if(fullname.value === "" || password.value === "" || email.value === "" || userName.value === ""){
...
const response = await fetch('/api/get_super_admin', {
...
if(data.message === "Log In Sucessful"){
    ...
    if(data.author.role === "superadmin"){
